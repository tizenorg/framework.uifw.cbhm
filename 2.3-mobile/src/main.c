...
static int app_create(void *data)
{
   ...
   elm_app_base_scale_set(1.8);
	...
	ad->color_theme = ea_theme_color_table_new("/usr/share/tables/_ChangeableColorInfo.xml");
	if (ad->color_theme)
		ea_theme_colors_set(ad->color_theme, EA_THEME_STYLE_LIGHT);
	else
		ERR("Error theme");

	ad->font_theme = ea_theme_font_table_new("/usr/share/tables/_ChangeableFontInfo.xml");
	if (ad->font_theme)
		ea_theme_fonts_set(ad->font_theme);
	else
		ERR("Error font");
...
static int app_terminate(void *data)
{
	...
	if (ad->color_theme)
	{
		ea_theme_color_table_free(ad->color_theme);
		ad->color_theme = NULL;
	}

	if (ad->font_theme)
	{
		ea_theme_font_table_free(ad->font_theme);
		ad->font_theme = NULL;
	}
	item_clear_all(ad);
